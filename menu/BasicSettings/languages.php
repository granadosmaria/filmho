...
// Agregar un nuevo idioma si se envió el formulario
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['name']) && isset($_FILES['image'])) {
    ...
    // Preparar y ejecutar la consulta para insertar el nuevo idioma
    $sql = "INSERT INTO language (name, image) VALUES ('$name', '$image')";

    if ($conn->query($sql) === TRUE) {
        echo "Nuevo idioma agregado correctamente.";
    } else {
...
if (isset($_POST['search'])) {
    ...
    $sql = "SELECT id, name, image FROM language WHERE name LIKE '%$search_query%'";
} else {
    $sql = "SELECT id, name, image FROM language";
}
...
<head>
    ...
    <title>Panel de Administrador - Languages</title>
...
<li class="submenu">
    ...
    <div class="main-content">
        <div id="content">
            <h1>Languages</h1>
            <!-- Formulario para agregar nuevos idiomas -->
            <form action="languages.php" method="POST" enctype="multipart/form-data">
                <label for="name">Name:</label>
                <input type="text" id="name" name="name" required>

                <label for="image">Image (Imagen):</label>
                <input type="file" id="image" name="image" accept="image/*" required>

                <button type="submit" class="btn-black">Add Language</button>
            </form>

            <!-- Barra de búsqueda -->
            <form method="POST" action="languages.php" class="search-form">
                <input type="text" name="search" placeholder="Buscar idioma..." value="<?php echo $search_query; ?>" class="search-input">
            ...
            <table border="1">
                ...
                <tbody>
                    <?php
                    // Mostrar los idiomas desde la base de datos
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><a href='editlanguage.php?id=" . $row['id'] . "'><i class='fas fa-edit'></i></a> | <a href='deletelanguage.php?id=" . $row['id'] . "'><i class='fas fa-trash'></i></a></td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='4'>No hay idiomas disponibles</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
