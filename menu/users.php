...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['name']) && isset($_POST['email']) && isset($_POST['mobile']) && isset($_POST['expiry_date']) && isset($_POST['type'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO user (image, name, email, mobile, expiry_date, type) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $image, $name, $email, $mobile, $expiry_date, $type);
    $stmt->execute();
...
if (isset($_POST['search'])) {
    ...
    $sql = "SELECT id, image, name, email, mobile, expiry_date, type FROM user WHERE name LIKE '%$search_query%' OR email LIKE '%$search_query%'";
} else {
    $sql = "SELECT id, image, name, email, mobile, expiry_date, type FROM user LIMIT $offset, $items_per_page";
}
    ...
    <div class="main-content">
        <div id="content">
            ...
            <form action="user.php" method="POST" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn-black">Agregar Usuario</button>
            </form>

            <form method="POST" action="user.php" class="search-form">
                <input type="text" name="search" placeholder="Buscar usuario..." value="<?php echo $search_query; ?>" class="search-input">
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>" . $row['type'] . "</td>";
                            echo "<td><a href='editUser.php?id=" . $row['id'] . "'><i class='fas fa-edit'></i></a> | <a href='deleteUser.php?id=" . $row['id'] . "'><i class='fas fa-trash'></i></a></td>";
                            echo "</tr>";
                        }
                    } else {
            ...
            <div class="pagination">
                <?php
                if ($page > 1) {
                    echo "<a href='user.php?page=" . ($page - 1) . "'>&laquo; Anterior</a>";
                }
                ...
                if ($page < $total_pages) {
                    echo "<a href='user.php?page=" . ($page + 1) . "'>Siguiente &raquo;</a>";
                }
                ?>
            </div>
        </div>
